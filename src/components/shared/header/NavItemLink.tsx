...
const getVariantClasses = (variant: NavItemVariant = 'default', isActive: boolean): string => {
  const variants = {
    default: isActive
      ? 'flex items-center gap-1 px-4 py-2 border border-stroke-2 dark:border-stroke-7 rounded-full bg-accent/20 dark:bg-transparent text-tagline-1 font-normal text-secondary dark:text-accent transition-all duration-200'
    ...
    border: isActive
      ? "flex items-center gap-1 h-full relative text-tagline-1 font-normal text-secondary dark:text-accent transition-all duration-200 overflow-hidden before:absolute before:bottom-0 before:left-0 before:h-px before:w-full before:scale-x-100 before:transition-transform before:duration-500 before:content-[''] dark:before:bg-white before:bg-secondary"
    ...
    white: isActive
      ? "flex items-center gap-1 h-full relative text-tagline-1 font-normal text-accent transition-all duration-200 before:absolute before:content-[''] before:bottom-0 before:left-0 before:h-px before:w-full before:bg-accent before:transition-all before:duration-300"
    ...
    light: isActive
      ? 'flex items-center gap-1 px-4 py-2 border border-stroke-1 dark:border-transparent rounded-full bg-accent/20 dark:bg-secondary/20 text-tagline-1 font-normal text-accent dark:text-accent transition-all duration-200'
